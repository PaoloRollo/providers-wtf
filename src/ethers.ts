...
export const toViemPublicClient = async (provider: providers.JsonRpcProvider): Promise<PublicClient | null> => {
...
export const toViemWsPublicClient = async (provider: providers.WebSocketProvider): Promise<PublicClient | null> => {
    const { connection } = provider
    const network = await provider.getNetwork()
    const { name } = network
    const chain = await import(`viem/chains/${name}`)
